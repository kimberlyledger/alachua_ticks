...
another way to code residual plots - this code is much cleaner that above...
```{r}
...
Will use a Generalized Least Squares (GLS) approach

start by defining weights matrix using several graph types - do i need to test more? 
```{r}
...
spdep::moran.test(aa_combined$PC1, listw.gab)
```



now test model for autocorrelation in the residuals 
```{r}
spdep::lm.morantest(nb.glm4, listw.gab) 
spdep::lm.morantest(nb.glm4, listw.rel) 
spdep::lm.morantest(nb.glm4, listw.neigh) 

```
no significant spatial autocorrelation in residuals... cool! 
...
plot(nb.glm7)
```
again sites 12 and 17 could be problematic... 


```{r}
spdep::lm.morantest(nb.glm7, listw.gab) 
spdep::lm.morantest(nb.glm7, listw.rel) 
spdep::lm.morantest(nb.glm7, listw.neigh) 

```
again no spatial autocorrelation! woo!  
...
try this cleaner way to plot model assumptions 

```{r}
par(mfrow=c(2,2), mar=c(4,4,2,1))
plot(nb.glm3)
```
sites 2 and 13 could be problematic... 
...
spdep::moran.test(is_combined$Forest1km, listw.gab)
spdep::moran.test(is_combined$PC1, listw.gab)
```
...
spdep::moran.test(is_combined$Forest1km, listw.rel)
spdep::moran.test(is_combined$PC1, listw.rel)
```
...
```{r}
spdep::lm.morantest(nb.glm3, listw.gab) 
spdep::lm.morantest(nb.glm3, listw.rel) 
spdep::lm.morantest(nb.glm3, listw.neigh) 
...
try a regular gls 
```{r}
gls0 <- gls(total ~ 1, data = is_combined, method = "REML")
gls1 <- gls(total ~ Forest2km , data = is_combined, method = "REML")
gls2 <- gls(total ~ Forested_Manicured, data = is_combined, method = "REML")
gls3 <- gls(total ~ Forest2km + Forested_Manicured, data = is_combined, method = "REML")
gls4 <- gls(total ~ Forest2km * Forested_Manicured, data = is_combined, method = "REML")
gls5 <- gls(total ~ PC1 , data = is_combined, method = "REML")
gls6 <- gls(total ~ PC1 + Forested_Manicured, data = is_combined, method = "REML")
gls7 <- gls(total ~ PC1 * Forested_Manicured, data = is_combined, method = "REML")

### compare AICc of models 
AICc(gls0, gls1, gls2, gls3, gls4, gls5, gls6, gls7)
```
okay, so with this gls4 and gls7 have lowest AICc


this doesn't seem to plot the residuals properly... 
```{r}
par(mfrow=c(2,2), mar=c(4,4,2,1))
plot(gls4)
plot(gls7)
...
check semivariance - should be a horizontal line at 1 
```{r}
semivario.gls4 <- nlme::Variogram(gls4, form = ~ Easting + Northing, resType = "normalized")
plot(semivario.gls4)
```

add correlation structure to models 
```{r}
mod.corExp <- update(gls4, correlation = nlme::corExp(form = ~ Easting + Northing, nugget=T))
mod.corGaus <- update(gls4, correlation = nlme::corGaus(form = ~ Easting + Northing, nugget=T))
mod.corSpher <- update(gls4, correlation = nlme::corSpher(form = ~ Easting + Northing, nugget=T))
mod.corRatio <- update(gls4, correlation = nlme::corRatio(form = ~ Easting + Northing, nugget=T))
#mod.corLin <- update(gls4, correlation = nlme::corLin(form = ~ Easting + Northing, nugget=T))

MuMIn::model.sel(gls4, mod.corExp, mod.corGaus, mod.corSpher, mod.corRatio)     
```

We refit the best model with maximum likelihood to test the fixed effects
```{r}
gls4.ML <- gls(total ~ Forest2km * Forested_Manicured, data = is_combined, method = "ML", correlation = nlme::corRatio(form = ~ Easting + Northing, nugget=T))
...
try a glm with glmmTMB - start with a basic negative binomial GLM 
...
tmb.nb1 <- glmmTMB(total ~ Forest2km , data = is_combined, family = nbinom2)
tmb.nb2 <- glmmTMB(total ~ Forested_Manicured, data = is_combined, family = nbinom2)
tmb.nb3 <- glmmTMB(total ~ Forest2km + Forested_Manicured, data = is_combined, family = nbinom2)
tmb.nb4 <- glmmTMB(total ~ Forest2km * Forested_Manicured, data = is_combined, family = nbinom2)
tmb.nb5 <- glmmTMB(total ~ PC1 , data = is_combined, family = nbinom2)
tmb.nb6 <- glmmTMB(total ~ PC1 + Forested_Manicured, data = is_combined, family = nbinom2)
tmb.nb7 <- glmmTMB(total ~ PC1 * Forested_Manicured, data = is_combined, family = nbinom2)

AICtab(tmb.nb0, tmb.nb1, tmb.nb2, tmb.nb3, tmb.nb4, tmb.nb5, tmb.nb6, tmb.nb7)
```
similar model results to glm.nb 


now add it spatial autocorrelation 
```{r}
...
# use Site ID dummy group factor to be used as a random term

tmb.nb0 <- glmmTMB(total ~ 1 + mat(pos + 0 |Site_ID), data = is_combined, family = nbinom2)
tmb.nb1 <- glmmTMB(total ~ Forest2km + mat(pos + 0 |Site_ID), data = is_combined, family = nbinom2)
tmb.nb2 <- glmmTMB(total ~ Forested_Manicured + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)
tmb.nb3 <- glmmTMB(total ~ Forest2km + Forested_Manicured + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)
tmb.nb4 <- glmmTMB(total ~ Forest2km * Forested_Manicured + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)
tmb.nb5 <- glmmTMB(total ~ PC1 + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)
tmb.nb6 <- glmmTMB(total ~ PC1 + Forested_Manicured + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)
tmb.nb7 <- glmmTMB(total ~ PC1 * Forested_Manicured + mat(pos + 0 | Site_ID), data = is_combined, family = nbinom2)

AICtab(tmb.nb0, tmb.nb1, tmb.nb2, tmb.nb3, tmb.nb4, tmb.nb5, tmb.nb6, tmb.nb7)
```

look at the models that converged
```{r}
summary(tmb.nb7)
summary(tmb.nb5)
summary(tmb.nb4)
...
```{r}
library(DHARMa)
sims <- simulateResiduals(tmb.nb7)
plot(sims)
```
